...
def replicar_formulas(user_inputs):
    ...
    try:
        ...
        peep = get_float('peep') 
        fio2 = get_float('fio2') # <- DEFINITION AÑADIDA
        ...
        if vo2 is not None and davo2 is not None and vo2 != 0.0 and davo2 != 0.0:
            ...
            gc_fick_calc = ((vo2 * 100) / davo2) / vo2 
            # MODIFICACIÓN SOLICITADA: Dividir el resultado total por 10
            gc_fick_calc = gc_fick_calc / 10
        
        # Reordenamos la inserción de resultados para incluir SatvO2 y GC Fick
        micro_resultados = {}
        if cao2 is not None: micro_resultados['CaO₂'] = f"{cao2:.2f} ml/dL"
        if cvo2 is not None: micro_resultados['CvO₂'] = f"{cvo2:.2f} ml/dL"
        if cco2 is not None: micro_resultados['CcO₂'] = f"{cco2:.2f} ml/dL"
        ...
        if davco2 is not None: micro_resultados['DavCO₂'] = f"{davco2:.1f} mmHg"
        if lactato is not None: micro_resultados['Lactato'] = f"{lactato:.2f} mmol/L"
        if satvo2 is not None: micro_resultados['SatvO₂'] = f"{satvo2:.1f} %" # ADICIÓN SOLICITADA
        if gc_fick_calc is not None: micro_resultados['GC Fick'] = f"{gc_fick_calc:.2f} L/min" # NUEVO PARAMETRO
        ...
        if rvs_pulm_in is not None: hemodinamia_resultados['RVSPulm. In.'] = f"{rvs_pulm_in:.2f} UW" # MODIFICACIÓN SOLICITADA
        ...
        if fio2 is not None: ventilatorio_resultados['FIO₂'] = f"{fio2:.2f}" # ADICIÓN SOLICITADA
        ...
        em_denominador = paco2
        em_numerador = (paco2 - peco2) if paco2 and peco2 else None
        em_calc = (em_numerador / em_denominador) * 100 if em_numerador and em_denominador and em_denominador != 0.0 else None
        if em_calc is not None: ventilatorio_resultados['EM'] = f"{em_calc:.2f} %" 
        
        ev_denominador = ((pi / 10) * 37.5) if pi else None
        ev_numerador = (paco2 * v_min) if paco2 and v_min else None
        ev_calc = ev_numerador / ev_denominador if ev_numerador and ev_denominador and ev_denominador != 0.0 else None
        if ev_calc is not None: ventilatorio_resultados['EV'] = f"{ev_calc:.2f}" 

        shunt_denominador = (cco2 - cvo2) if cco2 and cvo2 else None
        shunt_numerador = (cco2 - cao2) if cco2 and cao2 else None
        shunt_calc = ((shunt_numerador / shunt_denominador) * 100) if shunt_numerador and shunt_denominador and shunt_denominador != 0.0 else None
        ...
        if vt_ventilador is not None and fr is not None:
            ...
            if modo == "VCV":
                if ppico is not None and driving_p is not None:
                    term_p = ppico - (driving_p / 2) 
                    pm_calc = C1 * term_p
            elif modo == "PCV": 
                if driving_p is not None and peep is not None:
        ...
        # CÁLCULOS CORREGIDOS (Basados en CvJO₂ que se calcula usando SjO₂)
        ...
        # Usamos abs() para garantizar el valor positivo solicitado
        ...
        if ir_ab is not None: neuro_resultados['IR (AB)'] = f"{abs(ir_ab):.2f}" # Asumimos que (AR) se refiere a IR
        

        # --- SECCION 3: DTC (GENERICO) ---
        neuro_resultados['-- DTC (Genérico) --'] = " "
        
        # Mapeo corregido: Arteria Medida
